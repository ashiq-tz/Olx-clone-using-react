  ...
  return (
    <div>
        ...
        <nav className='fixed z-17 w-full overflow-auto p-2 pb-0 pl-3 pr-3 shadow-md bg-slate-100 border-b-4 border-solid border-b-white'>
            <img src={logo} alt="" className='w-12' onClick={() => navigate('/')}  />

            <div className='relative location-search ml-5'>
                <img src={search} alt="" className='absolute top-4 left-2 w-5' />
                <input type="text" placeholder='Search city,area,locality...' name='' id='' className='w-[50px] sm:w-[150px] md:w-[250px] lg:w-[270px] p-3 pl-8 pr-8 border-black border-solid border-2 rounded-md placeholder:text-ellipsis focus'/>
            ...
            <div className='ml-5 mr-2 relative w-full main-search'> 
                ...
                <div style={{backgroundColor:"#002f34"}} className='flex justify-center items-center absolute top-0 right-0 h-full rounded-e-md w-12'>
                    <img className='w-5' src={searchwt} alt="search icon" />
                </div>
            </div>
            ...
            <div className="flex items-center cursor-pointer" onClick={() => setDropdown(!dropDown)}>
                ...
                <img src={arrow} alt="" className={`w-5 transform transition-transform duration-200 ${dropDown ? 'rotate-180' : ''}`}/>
            </div>    
            </div>
        ...
        <div
            ...
            <p onClick={()=> { logoutUser(); }} className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Logout</p>
        </div>
